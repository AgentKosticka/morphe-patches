...
internal fun seekForwarderFingerprint(controlTypes: Set<String>) = Fingerprint(
    ...
    custom = { method, classDef -> classDef.fields.any { it.type in controlTypes } &&
        method.indexOfFirstInstruction(methodCall(parameters = method.parameterTypes.map { it.toString() }, returnType = "V")) >= 0 },
)
...
internal fun queueRemovalFingerprint(managerType: String, displays: QueueDisplaysAbi) = Fingerprint(
    ...
    custom = { method, _ ->
        method.indexOfFirstInstruction(fieldAccess(reference = displays.autoplay.managerField)) >= 0
    },
)

internal fun queueSnapshotFingerprint(storageType: String) = Fingerprint(
...
internal fun displayedQueueRefreshFingerprint(displayType: String) = Fingerprint(
    ...
    parameters = emptyList(),
    returnType = "V",
    filters = listOf(methodCall(name = "subList", parameters = listOf("I", "I"), returnType = "Ljava/util/List;")),
    custom = { method, _ ->
        method.indexOfFirstInstruction(methodCall("Ljava/util/List;->add(ILjava/lang/Object;)V")) >= 0 &&
            method.indexOfFirstInstruction(methodCall("Ljava/util/List;->remove(I)Ljava/lang/Object;")) >= 0
    },
)
...
internal fun queueCallbackConstructorFingerprint(callbackType: String, managerType: String) = Fingerprint(
...
internal fun clockOverrideColorsFingerprint(timeBarType: String, modelType: String) = Fingerprint(
    ...
    filters = listOf(
        literal(0),
        methodCall("Landroid/graphics/Paint;->setShader(Landroid/graphics/Shader;)Landroid/graphics/Shader;",
...
internal fun clockLabelEnabledFingerprint(timeBarType: String, modelType: String, dragging: MethodReference) = Fingerprint(
    definingClass = timeBarType,
    name = "draw",
...
internal fun BytecodePatchContext.queueStorageFingerprint(manager: ClassDef) = Fingerprint(
    parameters = listOf("I"),
    returnType = "Ljava/util/List;",
    custom = { _, owner ->
...
internal fun BytecodePatchContext.queueStateAccessorFingerprint(storageType: String, mode: Boolean) = Fingerprint(
    definingClass = storageType,
    parameters = emptyList(),
    returnType = if (mode) "L" else "I",
    custom = { method, owner ->
        (!mode || classDefByOrNull(method.returnType)?.let { AccessFlags.ENUM.isSet(it.accessFlags) } == true) &&
            owner.fields.any { field ->
                method.indexOfFirstInstruction(methodCall(
                    definingClass = field.type, parameters = emptyList(), returnType = method.returnType,
                    opcodes = listOf(Opcode.INVOKE_INTERFACE, Opcode.INVOKE_INTERFACE_RANGE),
                )) >= 0
            }
    },
...
internal fun clockModelTimestampFingerprint(modelType: String) = Fingerprint(
    definingClass = modelType, parameters = emptyList(), returnType = "J",
)
...
internal fun BytecodePatchContext.clockModelSetterFingerprint(timeBarType: String) = Fingerprint(
    ...
    custom = { method, owner ->
        owner.type in interfaceClosure(timeBarType) &&
            ...
            clockModelTimestampFingerprint(method.parameterTypes[0].toString()).matchAllOrNull().orEmpty().size >= 3
    },
)

internal fun clockDraggingFingerprint(timeBaseType: String) = Fingerprint(
    definingClass = timeBaseType, parameters = emptyList(), returnType = "Z",
)

internal fun BytecodePatchContext.clockMutableModelFingerprint(modelType: String) = Fingerprint(
    parameters = listOf("J", "J", "J", "J"),
    returnType = "V",
    custom = { _, owner -> !AccessFlags.ABSTRACT.isSet(owner.accessFlags) && implementsType(owner.type, modelType) },
)
...
internal fun accountRouterDispatchFingerprint(routerType: String) = Fingerprint(
    ...
    returnType = "V",
    filters = listOf(methodCall(parameters = listOf("L", "Ljava/util/Map;"), returnType = "V",
        opcodes = listOf(Opcode.INVOKE_INTERFACE, Opcode.INVOKE_INTERFACE_RANGE))),
    custom = { method, _ ->
...
internal fun protoDefaultInstanceFingerprint(messageType: String) = Fingerprint(
    ...
    name = "<clinit>",
    filters = listOf(fieldAccess(definingClass = messageType, type = messageType, opcode = Opcode.SPUT_OBJECT)),
)
...
internal fun protoParserFingerprint(messageHierarchy: Set<String>) = Fingerprint(
    ...
    returnType = "L",
    custom = { method, owner ->
        owner.type in messageHierarchy && method.returnType == owner.type &&
            method.parameterTypes[0].toString() == owner.type && AccessFlags.STATIC.isSet(method.accessFlags)
    },
)
...
internal fun queueItemEndpointFingerprint(ownerType: String, commandType: String) = Fingerprint(
    definingClass = ownerType, parameters = emptyList(), returnType = commandType,
)

internal fun BytecodePatchContext.queuePersistentIdFingerprint(itemTypes: Set<String>, commandType: String) = Fingerprint(
    parameters = emptyList(), returnType = "J",
    custom = { _, owner -> owner.type in itemTypes && queueItemEndpointFingerprint(owner.type, commandType).matchAllOrNull() != null },
)

internal fun queueTextAccessorFingerprint(ownerType: String) = Fingerprint(
    definingClass = ownerType, parameters = emptyList(), returnType = "Ljava/lang/String;",
)

internal fun BytecodePatchContext.queueVideoIdFingerprint(itemTypes: Set<String>) = Fingerprint(
    parameters = emptyList(), returnType = "Ljava/lang/String;",
    custom = { _, owner -> owner.type in itemTypes && queueTextAccessorFingerprint(owner.type).matchAll().size == 1 },
)

internal fun nowPlayingMetadataBindingFingerprint(textAccessors: List<MethodReference>) = Fingerprint(
    classFingerprint = WatchPageStateFingerprint,
    parameters = listOf("Lj$/util/Optional;"), returnType = "V",
    custom = { method, _ -> textAccessors.all { method.indexOfFirstInstruction(methodCall(reference = it)) >= 0 } },
)

internal fun queueRowRootFingerprint(rowType: String) = Fingerprint(
    definingClass = rowType, parameters = emptyList(), returnType = "Landroid/view/View;",
)

internal fun queueRowLongClickFingerprint(rowType: String) = Fingerprint(
    definingClass = rowType, parameters = listOf("Landroid/view/View;"), returnType = "Z",
)

internal fun queueMenuAccessorFingerprint(metadataType: String, menuType: String) = Fingerprint(
    definingClass = metadataType, parameters = emptyList(), returnType = menuType,
)

internal fun nowPlayingMenuDispatchFingerprint(entry: MethodReference) = Fingerprint(
    definingClass = entry.definingClass, name = entry.name,
    parameters = entry.parameterTypes.map { it.toString() }, returnType = entry.returnType,
    filters = listOf(methodCall(parameters = listOf("L", "Landroid/view/View;", "L", "L"), returnType = "V")),
)
...
internal fun queueItemConstructorFingerprint(itemTypes: Set<String>, factoryTypes: Set<String>) = Fingerprint(
    name = "<init>", parameters = listOf("J", "L", "L"),
    custom = { method, owner -> owner.type in itemTypes && method.parameterTypes[2].toString() in factoryTypes },
)

internal fun queueCallbackCaptureFingerprint(enqueue: Method, managerType: String) = Fingerprint(
    name = "<init>",
    filters = listOf(fieldAccess(type = managerType, opcode = Opcode.IPUT_OBJECT)),
    custom = { method, _ -> enqueue.indexOfFirstInstruction(methodCall(reference = method)) >= 0 },
)

internal fun BytecodePatchContext.queueSuccessCallbackFingerprint(callbackType: String) = Fingerprint(
    definingClass = callbackType, parameters = listOf("L"), returnType = "V",
...
internal fun queueMenuDispatcherFingerprint(dispatcherTypes: Set<String>, commandType: String) = Fingerprint(
    parameters = listOf(commandType, "Ljava/util/concurrent/Executor;"), returnType = "L",
...
internal fun queueMoveNotifierFingerprint(notifierType: String, commit: Method) = Fingerprint(
    definingClass = notifierType, parameters = listOf("L", "L"), returnType = "V",
    custom = { method, _ -> method.parameterTypes[0] == method.parameterTypes[1] &&
        commit.indexOfFirstInstruction(methodCall(reference = method)) >= 0 },
)

internal fun queueResponseMapperConstructorFingerprint(mapperType: String, providerTypes: Set<String>) = Fingerprint(
    definingClass = mapperType, name = "<init>",
    custom = { method, _ -> method.parameterTypes.any { it.toString() in providerTypes } },
)

internal fun BytecodePatchContext.queueResponseMapperFingerprint(mapperTypes: Set<String>, providerTypes: Set<String>) = Fingerprint(
    parameters = listOf("Ljava/lang/Object;"), returnType = "Ljava/lang/Object;",
    custom = { _, owner -> owner.type in mapperTypes &&
        queueResponseMapperConstructorFingerprint(owner.type, providerTypes).matchAllOrNull() != null },
)
...
internal fun queuePresenterRefreshFingerprint(ownerTypes: Set<String>, callers: Iterable<Method>) = Fingerprint(
    ...
    custom = { method, owner -> owner.type in ownerTypes && method.name != "<init>" && method.name != "<clinit>" &&
...
internal fun BytecodePatchContext.queueArtworkAccessorFingerprint(metadataType: String) = Fingerprint(
    definingClass = metadataType, parameters = emptyList(), returnType = "L",
    custom = { method, _ -> classDefByOrNull(method.returnType)?.fields?.count {
...
internal fun BytecodePatchContext.queueArtworkContractFingerprint(sharedTypes: Set<String>) = Fingerprint(
    ...
    custom = { method, owner -> owner.type in sharedTypes &&
...
internal fun BytecodePatchContext.queueMenuResponseConstructorFingerprint(responseTypes: Set<String>) = Fingerprint(
    name = "<init>",
    custom = { method, owner -> owner.type in responseTypes &&
        owner.fields.count { it.type == "Ljava/util/List;" || implementsType(it.type, "Ljava/util/List;") } == 1 &&
        method.parameterTypes.any { it.toString() == "Ljava/util/List;" || implementsType(it.toString(), "Ljava/util/List;") } },
)

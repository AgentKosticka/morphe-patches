...
public final class JamMetadata {
    ...
    public static void setText(TextView view, CharSequence text) {
        ...
        if (label == null) {
            ...
            boolean artist = "artist".equals(name) || "mini_player_subtitle".equals(name);
            if (title || artist) {
        ...
        if (!JamMirror.active()) label.local = TextUtils.stringOrSpannedString(text);
    ...
    static void refresh() {
        for (Map.Entry<TextView, Label> entry : new ArrayList<>(labels.entrySet())) {

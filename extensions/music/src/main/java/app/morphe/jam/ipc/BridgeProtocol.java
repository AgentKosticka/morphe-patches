...
public final class BridgeProtocol {
    ...
    private static final String[] FEATURES = {
            "queue-revisions", "stable-item-ids", "stale-edit-rejection"
    };

    private BridgeProtocol() {}

    public static JSONObject advertise(JSONObject message) throws JSONException {
        return message.put(KEY, new JSONObject().put("version", VERSION)
                .put("minimumVersion", VERSION).put("features", new JSONArray(Arrays.asList(FEATURES))));
    }
    ...
    public static JSONObject validate(JSONObject message) throws JSONException {
        ...
        if (minimum > version || minimum > VERSION || version < VERSION) {
            throw new IllegalArgumentException("Incompatible Jam bridge version; update Jam Layer and the Jam patch together");
        }
        Set<String> features = strings(protocol.getJSONArray("features"));
        if (!features.containsAll(Arrays.asList(FEATURES))) {
            throw new IllegalArgumentException("Jam bridge is missing required queue capabilities");
        }
        if (protocol.has("requires") && !new HashSet<>(Arrays.asList(FEATURES))
                .containsAll(strings(protocol.getJSONArray("requires")))) {
            throw new IllegalArgumentException("Jam bridge requires unsupported capabilities");
        }
        return message;
    }

    private static int positiveInteger(JSONObject object, String key) throws JSONException {
